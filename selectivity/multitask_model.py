...
from selectivity.nn.models import rcnn_wl_last
from selectivity.nn.nn import linearND
from selectivity.graph.ioutils_direct import binary_features_batch, binary_fdim
from selectivity.graph.mol_graph import atom_fdim as adim, bond_fdim as bdim, max_nb, smiles2graph_list as _s2g


class tf_predictor():
    def __init__(self, depth=5, hidden_size=300, batch_size=1, 
                model_path=os.path.join(os.path.dirname(__file__), 'model/model.ckpt-30615')):
        
        self.depth = depth
        self.hidden_size = hidden_size
        self.batch_size = batch_size
        with open(os.path.join(os.path.dirname(__file__), 'model/task_dict.pkl'), 'rb') as f:
...
if __name__ == "__main__":
    site_predictor = tf_predictor()
    site_predictor.build()
    #print(site_predictor.web_predictor(['Cc1ccccc1']))
    site_predictor.web_predictor(['Cc1ccccc1'])
    print(site_predictor.session)
    # builder = tf.saved_model.builder.SavedModelBuilder('site_model/1/') #has to have the 1 for version which is what tf serving will look for
    # builder.add_meta_graph_and_variables(site_predictor.session,[tf.saved_model.TRAINING])#,strip_default_attrs=True)
